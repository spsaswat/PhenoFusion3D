...
# Initialize your ROS node and other publishers/subscribers
rospy.init_node('robot_controller')
cmd_vel_publisher = rospy.Publisher('/cmd_vel', Twist, queue_size=10)
    
light_publisher = rospy.Publisher('/write_pin', UInt16, queue_size = 10)

joint_states_subscriber = rospy.Subscriber('/joint_states', JointState, joint_states_callback)

# position driver publisher
pub = rospy.Publisher('/go_to_position_server/goal', GotoActionGoal, queue_size=10)

# if not rospy.is_shutdown() and current_position >= 0.006:
#     go_home()


try:
    while not rospy.is_shutdown():
        # if capture_images is running, skip this iteration
        start_moving_robot()
